...
class DocumentChunker:
    ...
    def create_chunks_single(self, md_path, source_name=None):
        ...
        source_name = source_name or f"{doc_path.name}"
        
        # Fast 1:1 structured loader for MedQuad files
        if "medquad" in doc_path.name.lower() or doc_path.suffix.lower() == ".csv":
            from project.loaders.medquad_loader import load_medquad
            return load_medquad(doc_path, doc_id=doc_path.stem, source_name=source_name)
    ...
    def __rebalance_pair(self, first, second):
        ...
        if len(combined) > 2 * self.__max_parent_size:
            return first, second
        ...
        if not left_text or not right_text or len(left_text) > self.__max_parent_size or len(right_text) > self.__max_parent_size:
            return first, second
