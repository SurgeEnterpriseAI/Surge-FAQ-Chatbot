...
def orchestrator(state: AgentState, llm_with_tools):
    ...
    # Inject tool results if any tool was executed in this turn
    tool_results = state.get("tool_results", [])
    if tool_results:
        tool_context = "\n".join(f"- Tool: {tr['tool']}\n  Result: {tr['result']}" for tr in tool_results)
        summary_injection.append(HumanMessage(content=f"[TOOL CALL RESULTS FROM CONCURRENT EXECUTION]\n\n{tool_context}"))
